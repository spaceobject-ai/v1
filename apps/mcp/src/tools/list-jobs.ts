...
export const registerListJobsTool = (client: ApiClient) => (server: McpServer) => {
  server.registerTool(
    "list_jobs",
    {
      title: "List jobs",
      description:
        "List jobs, optionally filtered by client or provider address. Each job includes its status, participants, budget, description, timestamps, and activity history. Paginate with limit and skip.",
      inputSchema: {
